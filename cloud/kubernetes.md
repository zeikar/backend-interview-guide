...
- **특징:** 동적으로 변경되는 파드 IP 대신 안정적인 가상 IP와 DNS 이름을 제공하여, 클라이언트가 파드 변경을 직접 추적하지 않도록 해줍니다.
...
- **특징:** ConfigMap은 일반 설정 정보를, Secret은 민감 정보를 저장하기 위한 객체입니다. 다만 Secret 값은 기본적으로 base64 인코딩되어 저장되며, etcd 암호화는 별도 설정이 필요합니다.[^k8s-secret]
