...
1. **TLS 설정**
   ...
   - 운영 환경에서는 보통 TLS를 함께 구성하지만, gRPC 자체가 항상 TLS를 강제하는 것은 아닙니다. 언어/환경에 따라 insecure channel 또는 별도 transport security 구성을 사용할 수도 있으므로, "HTTP/2 = 반드시 TLS"로 단정하지 않는 것이 좋습니다.[^grpc-auth]
...
| **특징**           | **gRPC**                           | **REST**                            |
| ------------------ | ---------------------------------- | ----------------------------------- |
| **프로토콜**       | 표준적으로 HTTP/2 사용             | 주로 HTTP 기반이며 HTTP/1.1, HTTP/2 등과 함께 사용 가능 |
