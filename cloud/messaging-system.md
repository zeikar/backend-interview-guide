...
1. **로그 기반 메시징**:
   ...
   - 메시지를 로그 형태로 저장하고 복제를 통해 내구성을 높일 수 있어, 대량의 데이터를 처리하는 환경에 적합합니다.
...
4. **높은 처리량**:
   - Kafka는 파티션과 브로커를 통해 높은 처리량을 확보할 수 있어, 고성능 데이터 스트리밍 애플리케이션에 적합합니다.
...
3. **경량 메시지 처리**:
   ...
   - RabbitMQ는 개별 메시지 단위의 라우팅과 ack 기반 처리에 강점이 있어, 작업 큐(Task Queue) 처리에 적합합니다.
...
| **메시지 저장**           | 로그 기반 저장과 복제를 통해 내구성 확보 | 큐 설정에 따라 메모리 또는 디스크 기반으로 운용 가능 |
| **확장성**                | 브로커, 파티션 기반으로 수평 확장 가능   | 클러스터 기반 확장 (페더레이션으로 복잡도 증가) |
| **성능**                  | 높은 처리량에 강함                       | 낮은 지연 시간과 유연한 라우팅에 강함           |
| **신뢰성**                | 설정에 따라 높은 내구성과 재처리 보장 가능 | 기본적으로 ack/confirm 기반의 at-most-once 또는 at-least-once 모델을 주로 사용 |
...
1. **메시지 전달 보장 수준**:
   ...
   - **Exactly-once:** 시스템 전체에서 구현하려면 브로커 기능만으로는 부족하고, 애플리케이션의 idempotency 또는 중복 제거 전략이 함께 필요합니다.[^rabbitmq-reliability]
