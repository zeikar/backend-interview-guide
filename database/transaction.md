...
- **Repeatable Read:** 트랜잭션 동안 동일한 행을 여러 번 읽을 때 같은 결과를 얻도록 설계된 수준입니다. 다만 Phantom Read 허용 여부는 데이터베이스 구현에 따라 차이가 있을 수 있습니다.
...
- **Phantom Read:** 트랜잭션 내에서 동일한 조건의 쿼리를 실행할 때, 중간에 다른 트랜잭션이 새로운 데이터를 삽입하여 결과 집합이 달라지는 상황입니다. 일반적으로 Serializable에서 방지되며, 다른 격리 수준에서는 DBMS 구현에 따라 다르게 나타날 수 있습니다.

# 분산 트랜잭션(Distributed Transaction)

분산 트랜잭션(Distributed Transaction)은 두 개 이상의 독립적인 데이터베이스 시스템 또는 서비스에 걸쳐 수행되는 트랜잭션을 의미합니다. 단일 데이터베이스 내 트랜잭션과 달리 여러 시스템 간 동기화가 필요하며, 경우에 따라 2PC 같은 강한 일관성 기법을 사용하거나, Saga처럼 보상 트랜잭션 기반 접근을 선택하기도 합니다. 이러한 이유로 분산 트랜잭션은 관리와 구현이 매우 복잡하며, 성능 저하를 유발할 수 있습니다.
