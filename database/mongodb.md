...
- **유연한 스키마:** 컬렉션 내 문서 구조가 완전히 동일할 필요는 없습니다. 다만 "스키마가 없다"보다 "애플리케이션 차원에서 관리해야 한다"가 더 정확합니다.
...
MongoDB를 AP 시스템, MySQL을 CP 시스템처럼 단순 분류하면 면접에서 쉽게 꼬입니다.  
둘 다 실제 특성은 배포 구성과 읽기/쓰기 보장 설정에 따라 달라집니다.[^mongodb-causal]

MongoDB 쪽은 다음처럼 설명하는 편이 안전합니다.

- 문서 모델과 수평 확장성이 강점
- 강한 일관성도 일부 설정으로 확보 가능하지만 비용이 따른다
- 조인과 다중 문서 트랜잭션이 많아질수록 문서 지향 모델의 이점이 줄어든다
...
- **MongoDB를 단순히 AP로 고정해 설명하면 위험합니다.**
- **답변에서는 문서 모델, replica set, sharding, read/write concern까지 같이 말하는 편이 좋습니다.**
