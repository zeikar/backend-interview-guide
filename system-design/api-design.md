...
이 문서는 **계약 모델과 인터페이스 경계**에 집중합니다. 레이트 리밋의 기준 단위와 알고리즘은 [레이트 리미팅 (Rate Limiting)](rate-limiting.md), timeout/retry/circuit breaker 정책은 [복원력 패턴 (Timeout, Retry, Circuit Breaker)](resilience-patterns.md), 포맷 호환성은 [직렬화 (Serialization)](../programming/serialization.md) 문서를 함께 보면 더 직접적입니다.
...
관련 세부 주제는 [gRPC](../cloud/grpc.md) 문서와 함께 설명하면 좋습니다. JSON, Protocol Buffers, schema evolution 같은 포맷 관점은 [직렬화 (Serialization)](../programming/serialization.md) 문서가 더 직접적입니다.
...
HTTP 메서드 의미와 비즈니스 멱등성을 어떻게 나눌지, `Idempotency-Key` 저장 전략을 더 깊게 보려면 [멱등성과 재시도 (Idempotency and Retry)](idempotency-and-retry.md) 문서를 같이 보면 좋습니다. timeout budget, retryable error, circuit breaker 같은 복원력 일반론은 [복원력 패턴 (Timeout, Retry, Circuit Breaker)](resilience-patterns.md) 문서와 연결하면 좋습니다.
...
레이트 리밋의 기준 단위, burst 허용, `429`와 `Retry-After` 같은 상세 정책은 [레이트 리미팅 (Rate Limiting)](rate-limiting.md) 문서가 더 직접적인 심화 자료입니다.
